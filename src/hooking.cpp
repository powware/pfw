...
{
	ProtectGuard::ProtectGuard(void *address, std::size_t size) : address_(address), size_(size)
	{
	...
	{
		VirtualProtect(address_, size_, original_protection_, 0);
	}

	Nop::Nop(void *address, std::size_t size) : address_(address), original_opcode_()
	{
	...
	InlineHook::InlineHook(void *address, void *hook) : address_(address)
	{
	...
	VTableFunctionOverride::VTableFunctionOverride(void *address, void *hook) : address_(address)
	{
	...
	VTableFunctionOverride::~VTableFunctionOverride()
	{
	...
	TrampolineHook::TrampolineHook(void *splice, void *function_hook, std::size_t splice_size, bool push_entry_point) : splice_(splice), function_hook_(function_hook), splice_size_(splice_size)
	{
	...
	void *TrampolineHook::GetTrampoline()
	{
		return trampoline_;
	}

	VTableHook::VTableHook(void *class_pointer, std::size_t table_offset, std::size_t table_size)
	...
	void *VTableHook::AttachHook(void *function_hook, std::size_t table_entry_index)
	{
	...
	void VTableHook::DetachHook(std::size_t table_entry_index)
	{
